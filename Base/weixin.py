...
@Error.register
class WeixinErrors:
    UPDATE_WEIXIN_ACCESS_TOKEN_ERROR = Error("更新微信Access Token错误", code=Code.BadRequest)
    UPDATE_WEIXIN_JSAPI_TICKET_ERROR = Error("更新微信JsApi Ticket错误", code=Code.BadRequest)
    UPDATE_WEIXIN_TIME_NOT_EXPIRED = Error("更新间隔太短", code=Code.BadRequest)
...
class Weixin:
    ...
    def update_access_token():
        ...
        if crt_time - last_update < 60 * 80:  # 80 mins
            raise WeixinErrors.UPDATE_WEIXIN_TIME_NOT_EXPIRED
        ...
        if ('errcode' in data and data['errcode'] != 0) or 'access_token' not in data:
            raise WeixinErrors.UPDATE_WEIXIN_ACCESS_TOKEN_ERROR
        ...
        if ('errcode' in data and data['errcode'] != 0) or 'ticket' not in data:
            raise WeixinErrors.UPDATE_WEIXIN_JSAPI_TICKET_ERROR
